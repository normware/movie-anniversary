...
function generateFilterBarHTML(anniversaryValues, minYear, maxYear, pageType) {
  ...
  const extraGroups = pageType === 'week'
    ? `<div class="filter-group">
        <label class="option-chk"><input type="checkbox" id="expand-rows">Expand rows</label>
      </div>`
    : `<div class="filter-group">
...
(function(){
  ...
  function toggleExpand(expand){
    sections.forEach(function(s){
      ...
      if(row) row.classList.toggle('expanded', expand);
    });
  }
  ...
  function saveState(){
    var state = {
      ...
      expandRows: document.getElementById('expand-rows').checked
    };
  ...
  function loadState(){
    ...
    if(state.expandRows !== undefined) document.getElementById('expand-rows').checked = state.expandRows;
    toggleExpand(document.getElementById('expand-rows').checked);
  }
  ...
  document.getElementById('expand-rows').addEventListener('change', function(){
    toggleExpand(this.checked);
    saveState();
  });
  document.getElementById('reset-filters').addEventListener('click', function(){
    ...
    document.getElementById('expand-rows').checked = false;
    toggleExpand(false);
    applyFilters();
  });
